...
SCOPAS optimizes **ground-based heterogeneous sensor networks** (Radar, RF, EO) for sUAS surveillance in 3D urban / airport scenes. It is:

| Layer | Role |
|-------|------|
| Scene | GeoJSON buildings + candidate sites → voxel grid (+ optional DEM) |
| Physics | Ray-traced LoS, FOV/elevation/range gates, per-modality \(P_D\) |
...
RF covers **cooperative / Remote-ID** targets; Radar/EO cover **dark** targets. Fused resilience measures voxels where **both** modalities succeed under the threat map. Empirical runs already show the key research story:

- Coop-only fronts reach high \(M_{wp,coop}\) cheaply.
...
Reproducible configs, split-objective pipeline, hypervolume, OSM/DEM path, external-optimizer API (`evaluate_solution`), and documented case studies (synthetic city + SJC airport) are stronger than many one-off research scripts.
...
| No measurement validation | No comparison to real radar/RF field data or flight-test \(P_D\) |
...
3. **Operational evidence** that coop-optimized plans systematically under-protect dark targets; split fronts are required for honest safety claims.
4. **Open, reproducible planning toolchain** (GeoJSON scenes, NSGA-II/III, analysis pipeline) enabling external algorithms via a stable evaluation API.
...
4. **Ablations:** voxel resolution; Radar FOV 360° vs sector (120°); with/without site activation cost; RF present vs Radar+EO only; threat-map on/off.
5. **Reporting:** hypervolume (normalized), attainment surfaces, cost-to-reach threshold curves, flight-level \(M_c\), and **failure to reach** non-coop thresholds (important result).

Do not expand to more scenes until this matrix is clean and reproducible.

### Phase C — Strengthen the method just enough

Prioritize paper-critical upgrades only:

1. **Sensitivity & uncertainty:** sweep sensor params / resolution; report intervals, not single points.
...
3. **Problem & metrics:** dual-layer formulation, threat map, site CapEx, fused resilience definition.
4. **Method:** voxel LoS, \(P_D\) models (state assumptions clearly), NSGA-II encoding, objective profiles.
5. **Experiments:** city + SJC; split fronts; baselines; ablations.
...
5. Write related-work table differentiating 2023 SCOPAS vs this dual-layer CapEx extension.
