...
Compared to the original MUSCAT framework \cite{muscat2023} and related AAM surveillance placement models \cite{aievola2022ground,dulia2024sand} (Table~\ref{tab:related_comparison}), SCOPAS keeps the metric lineage while replacing exhaustive enumeration with NSGA-II in a \(g \cdot p\) budget \cite{deb2002fast}. Exhaustive search remains intractable for \(n\gtrsim25\) sites; city-scale runs therefore report Pareto fronts, not optimality gaps. Matched quality evidence is confined to Control~A (Table~\ref{tab:oracle_comparison}).
...
\textbf{Data Accessibility}: OSM integration provides practical accessibility for building-resolved LoS planning \cite{osm_applications,elias2023exploring}:
\begin{itemize}
...
\item \textbf{Modality Mix}: For classic \(M_c\) / coop-only floors, start RF-heavy (Controls~B--C). For dark-target or joint floors, budget EO/Radar first at lean CapEx, then heterogeneous mixes near \$320k (Control~C).
