...
Controls~A--B close two review blockers under matched physics: small-$n$ GA gaps stay \(\leq0.3\)\,pp versus oracle \(M_c\), and matched-budget ablation shows RF-only wins lean/mid classic coverage while heterogeneous mixes only edge at \$150k. City-scale dual-layer optimality gaps remain unsupported; heterogeneity stays a conditional, metric-dependent claim.
...
Control~B shows RF-only can dominate CapEx-matched classic \(M_c\) at lean/mid budgets; heterogeneous mixes help mainly when dark-target modalities or higher CapEx enter the plan. Diversification still matters for failure-mode resilience (weather vs.\ RF interference vs.\ acoustic clutter), but it is a conditional engineering choice---not a universal Pareto law on \(M_c\).
