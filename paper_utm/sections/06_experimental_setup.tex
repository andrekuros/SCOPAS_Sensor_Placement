...
Control~C repeats the matched-budget gates under threat-weighted dual-layer metrics \(M_{wp,coop}\) and \(M_{wp,noncoop}\) (same ray-tracer and five critical assets as the city dual-layer runs). Budgets \(\{\$31\mathrm{k},\$76\mathrm{k},\$150\mathrm{k},\$320\mathrm{k}\}\) (hardware only; \$320k matches the practical joint-floor CapEx band). For each modality$\times$budget we report (i)~the best-coop solution and its companion noncoop, and (ii)~the best-noncoop solution and its companion coop. Heterogeneity is claimed for a layer only when it wins that layer at equal CapEx.
